...
| Profile | Query |
|---------|-------|
| `profile.chatbase.json` | `AI customer support` |
| `profile.simular.json` | `computer-use agents` |
| `profile.slite.json` | `AI knowledge base` |
| `profile.property-saviour.json` | `UK property market` |
| `profile.clearnym.json` | `data broker removal` |

Each profile also includes `feed_urls` selected from the shipped RSS catalog. Profile RSS feeds are included by default.

Run the monitoring engine from this fixture directory. Start with a mock pass:

```bash
python3 ../../skills/newsjack-detector/scripts/newsjack_detector.py run "AI customer support" --profile profile.chatbase.json --mock --emit json
```
...
```bash
python3 ../../skills/newsjack-detector/scripts/newsjack_detector.py run --profile profile.chatbase.json --feed-only --emit brief
```

To simulate the hourly product behavior with duplicate suppression:

```bash
python3 ../../skills/newsjack-detector/scripts/newsjack_detector.py run --profile profile.chatbase.json --feed-only --save --new-only --max-age-hours 48 --emit brief
```

Then, if `MEDIALYST_API_KEY` is available and `xurl whoami` succeeds, run a live quick pass:

```bash
python3 ../../skills/newsjack-detector/scripts/newsjack_detector.py run "AI customer support" --profile profile.chatbase.json --sources news_search,x --lookback-days 7 --depth quick --save --emit json
```
...
- Source coverage: news_search / x / RSS feeds used or missing.
...
- Mention X tuning only when relevant: low-reach posts are filtered before ranking; useful X knobs are `NEWSJACK_X_MIN_ENGAGEMENT`, `NEWSJACK_X_MIN_AUTHOR_FOLLOWERS`, `NEWSJACK_X_MIN_VIEWS`, and the `NEWSJACK_X_TREND_*` thresholds.
```
