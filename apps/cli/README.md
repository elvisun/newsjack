...
The Go CLI is a thin client for Medialyst news search, PR calendar lookup, journalist enrichment, and automated media-list jobs plus local install, auth, update, skills, and monitor plumbing. It does not run or configure an MCP server, and it does not expose the hosted spreadsheet-management surface.
...
[Automated media-list creation](https://medialyst.ai/docs/developers/automated-media-lists) is asynchronous and credit-bearing. Agents must get explicit user approval for the exact campaign prompt and calculated research target before creation, then poll the returned job and surface normalized rows as they become ready:
