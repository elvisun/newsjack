...
func doctorWarnings(rootErr error, medialystConfigured, xConfigured bool) []string {
	...
	if !medialystConfigured {
		warnings = append(warnings, "Medialyst is not connected; live news search, journalist enrichment, and automated media lists will be unavailable. Run: newsjack login. For API-key automation, use: newsjack auth set-medialyst --key <mlst_...>.")
	}
	if !xConfigured {
...
func doctorActions(rootErr error, medialystConfigured, xConfigured bool) []map[string]string {
	...
	if !medialystConfigured {
		actions = append(actions, map[string]string{
			...
			"used_for":    "live news search, journalist enrichment, and automated media lists",
